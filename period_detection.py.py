...
def main():
    try:
        ...
        random_numbers = []
        iterations = []
        seen_numbers = set()  # To detect period by tracking unique random numbers
        period = 0
        ...
        plt.xlim(left=0)  # Ensure the x-axis starts at 0 exactly
        plt.grid(True)

        # Adjust layout to ensure 0 on x-axis is on the y-axis
        plt.tight_layout()
        plt.show()
